...
This plugin needs more than the ordinary vault API to do its job, so here's exactly what it touches and why:

- **Shell execution.** It runs the `kobo` command-line tool (from [Cobalt](https://github.com/BandarLabs/Cobalt)) as a subprocess — the "Kobo CLI path" setting controls which binary that is. `kobo` is what actually opens an SSH connection to your reader; this plugin never talks to the device itself.
- **Network use.** All of that traffic stays on your local Wi-Fi network — nothing on the wider internet, no third-party service, no account. It reaches only the IP address you set in **Settings → Clippings Sync**, to push unread Web Clipper notes to the device and pull back which notes were marked read and what tags were added.
- **Filesystem access outside the vault API.** Pulling changes writes the export `kobo` retrieves to a temporary directory (via Node's `os.tmpdir()`), reads it back, and deletes it once merged — this is how the device's response reaches the plugin at all, since it arrives as a file, not vault content. Reading it any other way isn't possible: it's not inside the vault until `applyEdit` merges it into a note's frontmatter through Obsidian's own `processFrontMatter`.

No data leaves your own network, and nothing here is used for anything other than the sync described above.
